...
/mob/living/carbon/human/update_worn_undersuit()
	...
	if(istype(w_uniform, /obj/item/clothing/under))
		...
		if(!found_special_sprite)
			var/corresponding_shape = uniform.get_matching_bodyshape(src)
			if (!isnull(corresponding_shape))
				// we found a config for our body? that must mean the clothing doesnt fit. so just trust it
				var/autogenerated_icon = uniform.generate_autogen_worn_icon(corresponding_shape, OFFSET_UNIFORM)
...
/mob/living/carbon/human/update_worn_id()
	...
	if(wear_id)
		...
		//my_chest?.worn_id_offset?.apply_offset(id_overlay) // DOPPLER EDIT REMOVAL - Moved into get_updated_worn_icon
...
/mob/living/carbon/human/update_worn_back()
	...
	if(back)
		...
		if(!back_overlay)
			return
		//var/obj/item/bodypart/chest/my_chest = get_bodypart(BODY_ZONE_CHEST) // DOPPLER EDIT REMOVAL - Moved up
		//my_chest?.worn_back_offset?.apply_offset(back_overlay) // DOPPLER EDIT REMOVAL - Moved into get_updated_worn_icon
...
/mob/living/carbon/human/get_held_overlays()
	...
	for(var/obj/item/worn_item in held_items)
		...
		//
		//hands += hand_overlay
		if(HAS_TRAIT(src, TRAIT_FLOATING_HELD))// DOPPLER EDIT START
			if(!held_left)
				held_left = new(src)
